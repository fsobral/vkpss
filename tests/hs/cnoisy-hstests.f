...
C     COMMON ARRAYS
      ...
      double precision a(NMAX), b(NMAX), XEX(NMAX * NMAX)
...
C     LOCAL SCALARS
      ...
      character OPTM
      logical OUTPUT
      integer FCNT,MAXFCNT,FLAG,NCYCLES
...
C     SET PERTUBATION STRUCTURE
      ...
      do i = 1, n
         ...
         b(i) = 2.0D0 * acos(-1.0D0) * ncycles / max(1.0D-1, 
     +          min(1.0D0, u(i) - l(i)))
         
         a(i) = 1.0D-02! + 1.0D0 / b(i)
...
!     COMMON ARRAYS
      double precision a(NMAX), b(NMAX), X(NMAX)
